...
class XY {
    public static void main(String args[]) {
        ...
        for (int j = 0; j < cs.length; j++)
        {
            if (cs[j].per > 75)
            {
                System.out.println(" ROLL NO : " + cs[j].rno + " has Percentage : " +
                        cs[j].per + " in " + cs[j].sem + " semester with A grade");
                count++;
            }
        }
        System.out.println("\nSchool Wise\n-----------------------------------------\n");
        for (int j = 0; j < ss.length; j++) {
            if (ss[j].per > 75) {
                System.out.println(" ROLL NO : " + ss[j].rno + " has Percentage : " +
                        ss[j].per + " in " + ss[j].cname + " standard with A grade");
                count++;
            }
        }
